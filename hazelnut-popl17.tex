...
This paper defines Hazelnut, a {structure editor} based on a small bidirectionally typed lambda calculus that takes two more steps in this direction: Hazelnut's edit actions operate over terms that are not only syntactically well-formed but also statically and dynamically meaningful.  
Na\"ively, enforcing an injunction on ill-typed edit states would force programmers to construct terms in an unnatural ``outside-in'' manner. To avoid this problem, Hazelnut's {action semantics} automatically places terms assigned a type that is incompatible with the expected type into a \emph{hole}, which safely defers the type compatibility check until the hole is \emph{finished}. Interestingly, this mechanism is related to a mechanism that arises in the semantics of gradual type systems.

We intend Hazelnut to serve as a principled foundational calculus, rather than as an end-user tool itself. To that end, we demonstrate how Hazelnut's rich metatheory, which we have mechanized in Agda, guides an extension to Hazelnut. We also  discuss how Hazelnut's semantics lends itself to implementation as a functional reactive program. Our reference implementation is written using \lstinline{js_of_ocaml}.
