...
First, we must extend the type consistency relation as shown in Figure \ref{fig:sum-statics}, following the example of type consistency for arrow types in Figure \ref{fig:type-consistency}. Similarly, we will need a notion of a \emph{matched sum type} directly analagous to the notion of a matched arrow type defined in Figure \ref{fig:type-consistency}. 

The type analysis rules shown in Figure \ref{fig:sum-statics} are essentially standard, differing only in that rather than directly matching on the structure of sum types, we use the matched sum type judgement.
