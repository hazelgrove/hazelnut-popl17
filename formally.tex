...
The mechanization proves that this judgemental definition of type inconsistency is indeed the negation of type consistency.\todo{big if true!!!}
...
The rules for relative movement within Z-expressions are similarly straightforward. Movement is type-independent, so we defer to an auxiliary expression movement judgement in both the analytic and synthetic case:\todo{remove prevSib}
\begin{subequations}
\begin{equation}
\inferrule{
...
\begin{lemma}[Movement Erasure Invariance]\label{lemma:movement-erasure} \todo{make lemmas number with theorems} 
\begin{enumerate}[itemsep=0px,partopsep=0px,topsep=0px]
...
Another check is to establish \emph{reachability}, i.e. that it is possible, through a sequence of movement actions, to move the cursor from any position to any other position within a well-typed H-expression. 

This requires developing machinery for sequences of actions. There are two possibilities: we can either add a sequencing action, $\alpha; \alpha$, directly to the syntax of actions, or we can define auxiliary syntax for sequences of actions, $\bar{\alpha}$, and ``multi\-step'' action judgements. To keep the core of the action semantics small, we took the latter approach in Figure \ref{fig:multistep}.

\begin{figure}
$\mathsf{ActionList}$~~$\bar{\alpha} ::= \cdot ~\vert~ \alpha; \bar{\alpha}$\\
...
\end{mathpar}
\caption{Multistep Action Performance}
\label{fig:multistep}
\end{figure}
...
To prove Reachability, we break it into two lemmas. One...\todo{write this}
...
\subsubsection{Deletion} The $\aDel$ action replaces the cursor with an empty hole.

The type action rule is self-explanatory:
\begin{subequations}
\begin{equation}
  \inferrule{ }{
    \performTyp{
...
\subsubsection{Construction}\label{sec:construction} The construction actions, $\aConstruct{\psi}$, are used to construct terms of a shape indicated by $\psi$ into the program at or around the cursor.
