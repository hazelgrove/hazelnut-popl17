...
\emph{Structure editors} allow programmers to edit the tree structure of a program directly -- there are no syntax errors. This can have cognitive benefits, particularly for novice and end-user programmers (as evidenced by the popularity of structure editors like Scratch.) It also simplifies matters for tool designers, because they need not contend with malformed program text. %Structure editors for professional programmers are also under active development (e.g. \texttt{mbeddr}.)

This paper defines Hazelnut, a {structure editor} based on a small bidirectionally typed lambda calculus that takes this approach two steps further: Hazelnut's edit actions operate over terms that are not just syntactically well-formed but also statically and dynamically meaningful. 
Na\"ively, this injunction against type errors would force programmers to construct terms in an unnatural ``outside-in'' manner. To avoid this problem, Hazelnut's {action semantics} automatically places terms that have a type that is incompatible with the expected type into a \emph{hole}, which safely defers the type compatibility check until the hole is \emph{finished}. Interestingly, this mechanism is related to a mechanism that arises in gradual type systems.

We intend Hazelnut as a minimal foundational calculus, rather than as an end-user tool itself. To that end, we show how Hazelnut's rich metatheory, which we have mechanized in Agda, guides the definition of an extension to Hazelnut. We also briefly discuss how Hazelnut's semantics lends itself to implementation as a functional reactive web application. Our reference implementation is written using \lstinline{js_of_ocaml}.
...
\label{sec:hazel}
\todo{revise text}\todo{add let and ifz}Hazelnut is based on the simply-typed lambda calculus extended with a single base type, $\tnum$. Its major constituents, introduced by example in the previous section, are:
\begin{itemize}
...
Reachability\todo{state reachability theorem}
...
Constructibility\todo{state constructibility theorem}
\end{subequations}
...
TODO: \url{http://research.nii.ac.jp/~hu/pub/hosc07.pdf}
...
TODO: cite \url{http://cseweb.ucsd.edu/~lerner/pb.html}

TODO: cite \url{http://delivery.acm.org/10.1145/1060000/1056965/p1557-ko.pdf?ip=73.154.143.34&id=1056965&acc=AUTHOR-IZED&key=4D4702B0C3E38B35%2E4D4702B0C3E38B35%2E4D4702B0C3E38B35%2EC2CC0A83071F7605&CFID=780613712&CFTOKEN=33786941&__acm__=1462392463_f29fc98965004c61bfb1291d07756a23}
