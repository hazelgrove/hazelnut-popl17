...
performing an action produces a unique result. Formally, this would be
established as follows:
\begin{theorem}[Action Determinism]
  \label{thrm:actdet} All of the following hold:
  \begin{enumerate}[itemsep=0px,partopsep=0px,topsep=0px]
  ...
  \end{enumerate}
\end{theorem}
This is not a theorem of the system exactly as described so far. The reason
...
``fire'' due to action subsumption, but one corresponds to an over-eager
use of subsumption: it's intended to be a rule of last resort.

The mechanization defines a predicate on action derivations that bans
exactly these over-eager uses of action subsumption, gives a mapping from
any derivation into one that matches the source and action and satisfies
the predicate, and proves determinism as stated above but only for
derivations modulo that predicate. This fix is analogous to the difference
between a chaining and focused logic in proof theory, which correspond
closely with bidirectional type checking systems, and therefore is to be
expected.\cite{Simmons11tr}
